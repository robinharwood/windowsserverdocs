---
title: Network Device Enrollment Service Guidance
TOCTitle: Network Device Enrollment Service Guidance
ms:assetid: 4054279e-5ab9-4fb7-bf0f-ebf3bc65143c
ms:mtpsurl: https://technet.microsoft.com/en-us/library/hh831498.1(v=WS.11)
ms:contentKeyID: 45261808
ms.date: 10/03/2022
...
In addition to these requirements, it may be necessary to have a service principal name (SPN) set in Active Directory. The following guidance is intended to help you determine whether it is necessary to set an SPN in Active Directory. 
  ...
  - If using a single NDES server and its actual hostname (a most common scenario), the account does not need an SPN registered.
      ...
      - If using all other defaults (particularly around IIS kernel-mode authentication), proceed to the next section.
...
If you require over-the-air enrollment for mobile devices, see [Using a Policy Module with the Network Device Enrollment Service](dn473016\(v=ws.11\).md).

> [!NOTE]
> <BR>If you make configuration changes for NDES or to the certificate templates that are used by NDES, you must stop and restart NDES, IIS and the CA service.
